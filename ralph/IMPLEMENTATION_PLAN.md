...
- **KV data layer** (`src/lib/kv.ts`): Check storage with 90-day TTL via `kv.expire`, status read/write, incident list with `lset` for in-place resolution (scans up to 200 items), uptime percentage calculation, `getChecksForPeriod` using `zrange` with `{ byScore: true }`
